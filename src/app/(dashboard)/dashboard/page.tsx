...
export default async function DashboardPage() {
  const [customerCount, invoiceCount, unprintedCount, printedCount, latestInvoices] =
    await Promise.all([
      prisma.customer.count(),
      prisma.invoice.count(),
      prisma.invoice.count({ where: { printer: { in: [0, 1] } } }),
      prisma.invoice.count({ where: { printer: 2 } }),
  ...
  const stats = [
    ...
    },
    {
      ...
      value: unprintedCount.toLocaleString(),
      icon: AlertCircle,
      color: "text-red-600",
      bg: "bg-red-50",
      href: "/unprinted",
                  ...
                  <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <Link
                        ...
                        className="inline-flex px-3 py-1 border border-gray-300 rounded text-xs hover:bg-gray-100 transition-colors"
                      >
                        Invoice
                      </Link>
                    </td>
