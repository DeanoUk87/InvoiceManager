...
class InvoicesController extends Controller
{
    ...
    public function queries($fromdate=null,$todate=null,$customer=null,$invoice=null,$printer=null)
    {
        if(Auth::user()->hasAnyRole(['admin','admin2'])) {
            if ($fromdate || $customer || $invoice || $printer) {
                ...
                return Invoices::orderBy('customer_account','ASC');
            }
        }else{
    ...
    public function index(Request $request)
    {
        ...
        return view('archive.invoices.index',compact('customer','customerName','invoice_no','invoiceNo','fromDate','toDate','printer','invoice'));
    }
    /**
    ...
    public function getdata($fromdate=null,$todate=null,$customer=null,$invoice=null,$printer=null)
    {
        ...
        return Datatables::of($invoices)
           ...
           <a href="javascript:viod(0)" data-id="row-'. $invoices->invoice_id .'" onclick="editForm(\''.url('archive/invoices/edit').'\','.$invoices->invoice_id.')" class="btn btn-success btn-xs"><i class="fa fa-pencil"></i></a> 
           <a href="javascript:viod(0)" data-id="row-'. $invoices->invoice_id .'" onclick="deleteData(\''.url('archive/invoices/delete').'\','.$invoices->invoice_id.')" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></a> 
           </div>';
            })
    ...
    public function details($id){
        ...
        return view('archive.invoices.details', compact('invoices'));
    }
    ...
    public function insert(){
        ...
        return view('archive.invoices.create' );
    }
    ...
    public function edit($id){
        ...
        return view('archive.invoices.edit', compact('invoices' ));
    }
    ...
    public function exportPDF($fromdate=null,$todate=null,$customer=null,$invoice=null){
        ...
        $pdf = PDF::loadView('archive.invoices.print', compact('invoices'));
    ...
    public function importExportView(){
        return view('archive.invoices.import');
    }
    ...
    public function exportFile($type){
        $invoices = Invoices::all('sales_id','customer_account','invoice_number','invoice_date','due_date','date_created','terms','printer','po_number','num')->toArray();
    ...
    public function SelectAllGroup(Request $request, $date=null)
    {
        ...
        if(count($customers) > 0){
            foreach($customers as $prof){
                $this->InsertInvoice($prof->customer_account,$prof->invoice_number,$prof->invoice_date,Carbon::now(config('timezone'))->format('Y-m-d'),Carbon::now(config('timezone'))->format('Y-m-d'),'',$prof->sales_id);
            }
    ...
    public function invoice($customer_account, $invoice_number,$invoice_date,$print=null){
        ...
        return view('archive.invoices.invoice',compact('setup','customers','prof','items','subtotal','vat','customer_account','invoice_number','invoice_date','invo','mail','printer','owner'));
    }
    ...
    public function exportDetailPDF($customer_account, $invoice_number,$invoice_date){
        ...
        //$pdf->setPaper(array(0,0,620.78,600.94),'A4');
    ...
    public function exportDetailExcel($customer_account, $invoice_number,$invoice_date){
        ...
        $invTitle = 'Invoice '.$invoice_number;
        // Generate and return the spreadsheet
        Excel::create('invoice_'.$invoice_number, function($excel) use ($paymentsArray,$invTitle,$invoice_number) {
            // Set the spreadsheet title, creator, and description
            $excel->setTitle($invTitle);
            $excel->setCreator('Invoice Software')->setCompany('Hezecom');
            $excel->setDescription('Account Info');
            // Build the spreadsheet, passing in the payments array
            $excel->sheet('Invoice_'.$invoice_number, function($sheet) use ($paymentsArray) {
        ...
        if($customer_email){
            if(strpos($customer_email, ',') === false){
                if(filter_var($customer_email, FILTER_VALIDATE_EMAIL)){
                    Mail::to($customer_email)->send(new MassMail2($customer_account,$invoice_number,$invoice_date,$customer_email));
                } else{
            ...
            foreach($customers as $hmail){
                ...
                if(strpos($customer_email, ',') === false){
                    if(filter_var($customer_email, FILTER_VALIDATE_EMAIL)){
                        ...
                        $emailSent[]= '<span class="text-danger">Invalid email address '.$customer_email.'</span>';
                    }
                }
                else {
                    $custEmails = explode(',',$customer_email);
                    foreach($custEmails as $customerEmail){
                        if(filter_var($customerEmail, FILTER_VALIDATE_EMAIL)){
                            Mail::to($customerEmail)->send(new MassMail2($customer_account,$invoice_number,$invoice_date,$customerEmail));
                        }else{
                            $emailSent[]= '<span class="text-danger">Invalid email address '.$customerEmail.'</span>';
                        }
                    }
                }
                //Mail::to($customer_email)->send(new MassMail2($customer_account,$invoice_number,$invoice_date,$customer_email));
                ...
                //return redirect()->route('invoices.index')->with('success', 'All invoices has been sent to customers with email addresses.');
            }
            //print_r($emailSent);
            return view('archive.invoices.sent', compact('emailSent'));
        }else{
            return redirect()->route('invoices.index')->with('error', 'No new invoices found!');
        }
    }
