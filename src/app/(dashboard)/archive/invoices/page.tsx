import { db } from "@/db";
import { invoicesArchive } from "@/db/schema";
...
export default async function InvoicesArchivePage() {
  const archiveInvoices = await db.select().from(invoicesArchive).orderBy(desc(invoicesArchive.archivedAt)).limit(500);
  return (
          ...
          <tbody>
            {archiveInvoices.length === 0 ? (
              ...
              <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="px-4 py-3"><span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">{inv.printer===2?"Printed":"Unprinted"}</span></td>
                <td className="px-4 py-3 text-gray-500">{inv.archivedAt ? formatDate(inv.archivedAt.toISOString()) : "-"}</td>
              </tr>
            ))}
          </tbody>
        </table>
