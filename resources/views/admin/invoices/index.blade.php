                                ...
                                <div class="dropdown">
                                    ...
                                    @if($printer!=2)
                                        @if(Auth::user()->hasAnyRole(['admin','admin2']))
                                            <a href="{{route('send.preview')}}" class="btn btn-success">
                                                <span class="fa fa-send-o"></span> Send Mass Mail to Customers
                                            </a>
                                        @endif
                                    @endif
                                    ...
                                    <div class="dropdown-menu dropfix" aria-labelledby="dropdownOptions">
                                        <a class="dropdown-item" href="javascript:viod(0)" onclick="insertForm('{{route('invoices.create')}}')">@lang('app.create')</a>
                                        <a class="dropdown-item" href="{{ route('invoices.export',['type'=>'xlsx','fromdate'=>$fromDate,'todate'=>$toDate, 'customer'=>$customer,'invoice'=>$invoice_no]) }}">@lang('app.export.xlsx')</a>
                                        <a class="dropdown-item" href="{{ route('invoices.export',['type'=>'xls','fromdate'=>$fromDate,'todate'=>$toDate, 'customer'=>$customer,'invoice'=>$invoice_no]) }}">@lang('app.export.xls')</a>
                                        <a class="dropdown-item" href="{{ route('invoices.export',['type'=>'csv','fromdate'=>$fromDate,'todate'=>$toDate, 'customer'=>$customer,'invoice'=>$invoice_no]) }}">@lang('app.export.csv')</a>
                                        <a class="dropdown-item" href="{{route('invoices.pdf',['fromdate'=>$fromDate,'todate'=>$toDate, 'customer'=>$customer,'invoice'=>$invoice_no])}}">@lang('app.export.pdf')</a>
                                    </div>--}}
                                </div>
                            </div>
                        </nav>
...
<script type="text/javascript">
    var table = $('#invoices_datatable').DataTable({
        ...
        searching: true,
        "order": [[0, "asc" ]],
        ajax: "{{ route('invoices.search',['fromdate'=>$fromDate,'todate'=>$toDate, 'customer'=>$customer,'invoice'=>$invoice_no,'printer'=>$printer]) }}",
        columns: [
